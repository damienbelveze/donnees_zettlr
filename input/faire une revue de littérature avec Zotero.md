...
---
# Faire une revue de littérature avec les tags de Zotero
...
# Analyser les articles d'un corpus de références sur Zotero

Voir code de Pascal Martinolli pour [[R (logiciel)|R]] et pour [[observablehq]]. 
R : https://github.com/pmartinolli/ZoteroRnalysis
Observable: https://observablehq.com/@pascaliensis/zoterojsnalysis


# Formation de Frédérique Flammerie : Zotero pour les revues de littérature

formation du 25 avril 2025, URFIST Rennes

créer pour la revue de littérature une bibliothèque de groupe pour isoler les références liées à ce travail à part de ses références personnelles (importance pour le dédoublonnage)

choisir : "Choose public, Membership"
invitations 
envoyer l'adresse du groupe à ses partenaires. 
valider leur participation. 
Quand tous les membres prévus pour le groupe l'ont rejoint, remettre le groupe en "private"
Possibilité d'avoir du texte intégral

## Zoplicate

noter le nombre de doublons (cf. méthode PRISMA) AVANT de procéder au dédoublonnage

Zoplicate permet de charger dans la bibliothèque que les références qui ne s'y trouvent pas. 

"Master item préférences"  : duplicate fusionne (merge) tout tout seul, possibilité de charger la référence la plus détaillée, la plus récente ou celle qui a été rajoutée en premier. 

Utiliser Linter pour enlever les Capitales au titres de revues

## Linter

décocher "lint when added in the library" (pour éviter les lenteurs)

Titles should be sentenced cases + pas de point à la fin des titres

[[Format de date|Date]] au format ISO-8101

si on ne trouve pas que des revues anglaises, décocher titles should be capitalized (pour éviter que les noms des titres aient des majuscules)

Ctrl+I pour mettre des termes en italiques dans le titre
Ctrl+= pour mettre en exposant

# action tags

ajoute des tags quand le document a subi une action (ouverture par exemple du PDF)

action *unread* intégration d'un nouvel événement -> ajout d'un tag unread quand le fichier n'a pas été ouvert. 
action : append note -> tag lu et annoté

liste de scripts utilisable avec Actiontags :
https://github.com/windingwind/zotero-actions-tags/discussions/categories/action-scripts

par exemple 'bulk multiple items'


Les marqueurs sont plus pérennes que la situation dans une collection ; en cas d'export, les tags accompagnent les références par défaut, ce n'est pas le cas des collections. 

usage des recherches enregistrées (distinctes des collections)

Optimiser la recherche du texte intégral

## Créer un format d'export personnalisé en CSV

(pour simplifier l'export CSV)

Zotero > translators

Chercher le convertisseur CSV.js
Changer l'ID et le label du translator (comme pour un style biblio)
changer les variables qu'il faut dans la variable 'exported_fields'
